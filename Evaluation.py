...
def evaluation(actual, predict):
    ...
    npv = NPV(TN, FN)
    Confidence = TP / (TP + FP)
    Purity_Ratio = PURITY(TP, FP, FN)
    NMI_Ratio = NMI(TP, TN, FP, FN)
    Growth_Rate = (len(actual) * (FP + TN)) / (len(predict) * (TP + FN))
    Values = np.asarray([TP, TN, FP, FN, accuracy, Recall, precision, Confidence, Growth_Rate, Purity_Ratio, NMI_Ratio])
